...
class DaikinCloudDevice {
    ...
    _traverseDatapointStructure(obj, data, pathPrefix) {
        ...
        Object.keys(obj).forEach(sub => {
            ...
            if (sub === 'meta' || subKeys.includes('value') || subKeys.includes('settable') || subKeys.includes('unit') || (subKeys.includes('d') && subKeys.includes('w') && subKeys.includes('m'))) { // we found end leaf
