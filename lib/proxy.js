...
class DaikinCloudProxy {
     ...
     * @param {object} options.proxyOwnIp Own external IP of the proxy for the final redirect (displayed in the browser)
        ...
        try {
            ...
            if (mitmCaFile) {
                ...
                if (fileContent && fileContent.includes('.validity.notBefore.getFullYear() + 2);')) {
                    ...
                    this.options.logger && this.options.logger('http-mitm-proxy/lib/ca.js patched to only generate 1 year long certificates');
                } else {
                    this.options.logger && this.options.logLevel === 'debug' && this.options.logger('http-mitm-proxy/lib/ca.js already patched to only generate 1 year long certificates');
                }
            } else {
                this.options.logger && this.options.logger('http-mitm-proxy/lib/ca.js not found');
            }
        } catch (e) {
            this.options.logger && this.options.logger(`Cannot patch http-mitm-proxy/lib/ca.js: ${e}`);
        }
                        ...
                        <html lang="en">
                            <body>
                                ...
                                <b>
                                    ...
                                    <p/>
                                    IMPORTANT: If this process did not work please RELOAD this page before you do another try because the Link is only valid once!!
                                </b>
                            </body>
                        </html>
                    `;
    ...
    async _retrieveTokens(callbackUrl) {
        ...
        if (params.code) {
            ...
            const tokenSet = await this.openIdClient.grant(
                {
